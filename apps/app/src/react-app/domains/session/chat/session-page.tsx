...
import { Columns2, FileText, FolderTree, Globe, Mic2, Settings2, X, Zap } from "lucide-react";
...
export function SessionPage(props: SessionPageProps) {
  ...
  const explorerRailActive = activeSidePanel === "explorer";
  ...
  const openExplorerRailPane = useCallback(() => {
    toggleCurrentSidePanel("explorer");
        ...
        <SidebarInset className="min-h-0 overflow-hidden bg-background mac:bg-background/80 mac:[&_header]:transition-[padding-left] mac:[&_header]:duration-200 mac:[&_header]:ease-linear mac:peer-data-[state=collapsed]:[&_header]:pl-28 mac:max-md:[&_header]:pl-28">
          ...
          <aside className="flex w-11 shrink-0 flex-col items-center gap-1 border-l border-border bg-background/95 px-1 py-2 text-muted-foreground mac:titlebar-no-drag">
            ...
            <Button
              ...
              className={cn(
                "rounded-xl transition-colors hover:bg-muted hover:text-foreground",
                explorerRailActive && "bg-primary/10 text-primary hover:bg-primary/15 hover:text-primary",
              )}
              onClick={openExplorerRailPane}
              title="File Explorer"
              aria-label="File Explorer"
              aria-pressed={explorerRailActive}
            >
              <FolderTree size={17} />
            </Button>
            <Button
